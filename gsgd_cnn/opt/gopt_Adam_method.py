...
class gSGD_Adam_method():
    ...
    def cal_R_cnn(self, lr, w_red, dw_red, sigmadwd, v_value, w_norm, mask,exp_avg, exp_avg_sq):
        ...
        if self.args.adam_method =='weight':
            ...
            lr2 = (lr1 / ( torch.sqrt(exp_avg_sq) +  self.eps ) * mask).sum(1).sum(1).sum(1)

            res = 1 - lr2 *  ( exp_avg* mask).sum(1).sum(1).sum(1)/  v_value

        elif self.args.adam_method =='noweight':

            lr1 = lr * np.sqrt(bias_correction2) / bias_correction1

            lr2 = (lr1 / ( torch.sqrt(exp_avg_sq)*w_norm +  self.eps ) * mask).sum(1).sum(1).sum(1)

            res = 1 - lr2 * ( exp_avg * w_norm * mask).sum(1).sum(1).sum(1) / v_value
