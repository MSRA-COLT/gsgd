...
from opt.gopt_SGD_method import gSGD_SGD_method
...
class gSGD_SGD(gSGD_SGD_method):
    ...
    def cal_internal_optim(self):
        if self.args.inner_opt == 'sgd':
            self.internal_optim = SGD(self.models.parameters(), lr=self.param_groups[0]['lr_inner'], momentum=self.momentum,
                                      weight_decay=self.weight_decay, nesterov=False)
        elif self.args.inner_opt == 'adam':
            self.internal_optim = Adam(self.models.parameters(), lr=self.param_groups[0]['lr_inner'],
                                       betas=self.betas, eps=self.eps, weight_decay=self.weight_decay)
        elif self.args.inner_opt == 'rmsprop':
            self.internal_optim = RMSprop(self.models.parameters(), lr=self.param_groups[0]['lr_inner'])
        else:
            assert 1 == 2
            ...
            if blocks_type == 'cnn':
                ...
                for block in blocks_idx:
                    ...
                    for ec_layer_idx, layer_idx in enumerate(block):
                        ...
                        mask_tmp = self.generate_eye_cnn(outcome, income,shape_2, shape_3, loc).cuda()
                        layer.data = layer.data * (1 - mask_tmp)*self.args.blue_init  + mask_tmp *  red_init
                ...
                for block in blocks_idx:
                    ...
                    for ec_layer_idx, layer_idx in enumerate(block):
                        ...
                        mask_tmp = self.generate_eye_mlp(outcome, income, loc).cuda()
