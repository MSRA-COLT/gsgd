    ...
    def pre_step(self):
        ...
        for group in self.param_groups:
            for layer_idx, p in enumerate(group['params']):
                ...
                if group['weight_decay'] != 0:
                    grad = grad.add(group['weight_decay'], p.data * (1 - mask))
                ...
                # print((p.grad/group['lr'] ).data.abs().mean(),layer_idx,'!!!!!!!!!!!!!' )
    ...
    def partial_bp_step(self, closure=None):
        ...
        for group in self.param_groups:
            for layer_idx, p in enumerate(group['params']):
                ...
                # print((step_size*exp_avg/denom/group['lr'] ).abs().mean(), layer_idx, '###############')
...
#                 step_size = group['lr'] * math.sqrt(bias_correction2) / bias_correction1
#
#                 p.grad.data =  math.sqrt(bias_correction2)  * exp_avg / denom/ bias_correction1
#                 # p.data.addcdiv_(-step_size, exp_avg, denom)
#
#
#     def partial_bp_step(self, closure=None):
