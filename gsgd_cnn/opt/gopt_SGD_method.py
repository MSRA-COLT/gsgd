...
class gSGD_SGD_method():
    ...
    def adjust_learning_rate_multistep(self,  epoch,  decay_step, decay_gamma):
        """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
        # lr = args.lr * (0.1 ** (epoch // 30))
        # lr = args.lr * (args.decay_gamma_imagenet ** (epoch // 30))
        if (epoch + 1) in decay_step:
            lr = self.get_lr()[0] * (decay_gamma ** (decay_step.index(epoch + 1) + 1))
            self.set_lr(lr, lr)
            print('gsgd decay lr to :', lr)


    def recover_s_layer(self, value, idx, shape):
        assert value.device == idx.device
        if value.device.type == 'cpu':
            return torch.sparse.FloatTensor(idx, value, shape).to_dense().cuda()
        else:
            return torch.cuda.sparse.FloatTensor(idx, value, shape).to_dense().cuda()
    ...
    def cal_R_cnn(self, lr, w_red, dw_red, sigmadwd, v_value, w_norm, mask, exp_avg):

        if self.args.mom_method == 'weight':

            res = 1 - lr * (exp_avg * mask).sum(1).sum(1).sum(1) / v_value

        elif self.args.mom_method == 'noweight':

            res = 1 - lr * (exp_avg * w_norm * mask).sum(1).sum(1).sum(1) / v_value

        return (res)

    def cal_R_mlp(self, lr, w_red, dw_red, sigmadwd, v_value, mask, exp_avg, ):

        res = 1 - lr * (exp_avg * mask).sum(1) / v_value
        return (res)
    ...
    def ec_step_cnn(self, blocks_idx, closure=None):
        ...
        for block_idx, block in enumerate(blocks_idx):
            ...
            sigmadwd = torch.zeros(this_block_s_h).cuda()
            w_norm = []
                        ...
                        if self.args.mom_method == 'weight':
                            self.exp_avg['cnn'][block_idx][ec_layer_idx] = \
                                (mom *
                                 ...
                                 (dw_blue * w_norm[ec_layer_idx]
                                  / v_value[:out_shape].unsqueeze(1).unsqueeze(1).unsqueeze(1)
                                  + self.weight_decay * layer.data * v_value[:out_shape].unsqueeze(1).unsqueeze(1).unsqueeze(1)
                                  )) \
                            ...
                            self.exp_avg['cnn'][block_idx][ec_layer_idx] = \
                                (mom *
                                 self.exp_avg['cnn'][block_idx][ec_layer_idx]
                                 + (1 - 0) *
                                 (dw_blue / v_value[:out_shape].unsqueeze(1).unsqueeze(1).unsqueeze(1)
                                  + self.weight_decay * layer.data * v_value[:out_shape].unsqueeze(1).unsqueeze(1).unsqueeze(1))) \
                                * (1 - this_mask_sub_layer) + this_mask_sub_layer
                        ...
                        if self.args.mom_method == 'weight':
                            ...
                            self.exp_avg['cnn'][block_idx][ec_layer_idx] = \
                                (mom *
                                 ...
                                 + (1 - 0) *
                                 (dw_blue * w_norm[ec_layer_idx]
                                  / v_value[:in_shape].unsqueeze(1).unsqueeze(1).unsqueeze(0)
                                  + self.weight_decay * layer.data * v_value[:in_shape].unsqueeze(1).unsqueeze(1).unsqueeze(0))) \
                                * (1 - this_mask_sub_layer) + this_mask_sub_layer
                            ...
                            self.exp_avg['cnn'][block_idx][ec_layer_idx] = \
                                (mom *
                                 self.exp_avg['cnn'][block_idx][ec_layer_idx]
                                 + (1 - 0) *
                                 (dw_blue / v_value[:in_shape].unsqueeze(1).unsqueeze(1).unsqueeze(0)
                                  + self.weight_decay * layer.data * v_value[:in_shape].unsqueeze(1).unsqueeze(1).unsqueeze(0))) \
                                * (1 - this_mask_sub_layer) + this_mask_sub_layer
                    ...
                    if self.args.mom_method == 'weight':
                        ...
                        tmp_exp_avg_red = \
                            (mom *
                             self.exp_avg['cnn'][block_idx][ec_layer_idx]
                             + (1 - 0) *
                             (((w_red * dw_red - sigmadwd) / (v_value))).unsqueeze(1).unsqueeze(1).unsqueeze(1)) \
                            * this_mask_sub_layer
            ...
            for ec_layer_idx, layer_idx in enumerate(block):
                ...
                lr = lr_t
                weightornot = self.args.mom_method == 'weight'
